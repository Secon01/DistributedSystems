...
public class Master
{
    ...
    private void handleBookRoomRequest(BufferedReader in, OutputStream out) throws IOException, InterruptedException 
    {
        ...
        System.out.println("\n" + "Received book request for room: " + bookTuple.getRoomName());
        int workerID = hashFunc(bookTuple.getRoomName(), workerConfig.nofWorkers);                     // hash room name and get worker id to send request  
        int workerPort = getPort(workerID);                                             // get port of selected worker
        Thread book  = new Thread(() -> {
            ...
            try (BufferedReader inputWorker = new BufferedReader
                                        (new InputStreamReader(socket.getInputStream()))) {
                sendBookRoomRequest(outputWorker, jsonBook);                                   // send book request
                // Read the response
                String responseLine;
