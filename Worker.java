...
public class Worker
{
    ...
    private void handleBookRoomRequest(BufferedReader in, OutputStream out) throws IOException {
        String jsonBook = extractBody(in);                                                                            // extract json with room name from request body
        Booking bookTuple = deserializeBooking(jsonBook);                                       // create string with room name from json 
        //String date1 = "2024-04-07";
        //String date2 = "2024-04-10";
        //String date1 = new Gson().fromJson(jsonRoomName, String.class);
        //String date2 = new Gson().fromJson(jsonRoomName, String.class)
        System.out.println(jsonBook.toString());
        System.out.println("\nReceived request for booking: " + bookTuple.getRoomName() + "\n");       
        if(book(bookTuple)) {                                                                                              // book room by name and check if it is booked
            sendHttpResponse(out, 200, "OK", 
                            "{\"message\":\" " + bookTuple.getRoomName() + " booked\"}",
                "application/json");                                                                         // send response for successful http request 
        } else {
            sendHttpResponse(out, 409, "Conflict", 
                            "{\"message\":\" " + bookTuple.getRoomName() + " already booked\"}", 
                "application/json");                                                                        // send response for unsuccessful http request
        }
    }
